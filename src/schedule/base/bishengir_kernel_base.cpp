...
public:
    Status Init(const LaunchParam &launchParam, const RunInfo &runInfo, KernelInfo &kernelInfo)
    {
        ...
        status = UpdateWSArgs(args, launchParam, kernelInfo, kernelArgsIndex);
        MKI_CHECK(status.Ok(), "failed to update workspace args", return status);

        status = UpdateTilingArgs(args, launchParam, kernelInfo, kernelArgsIndex);
...
private:
    ...
    Status UpdateWSArgs(void **args, const LaunchParam &launchParam, KernelInfo &kernelInfo,
        const MiniVector<uint64_t> &kernelArgsIndex) const
    {
        auto wsIndex =
        ...
        for (size_t i = 0; i < kernelInfo.GetScratchSizes().size() && wsIndex < argsNum; i++) {
            MKI_LOG(DEBUG) << "args info: output tensor " << wsIndex;
            auto idxArgs = kernelArgsIndex[wsIndex];
            size_t workspaceSize = kernelInfo.GetScratchSizes()[i];
            // 申请空间ss
            auto ret = Mki::MkiRtMemMallocDevice((void **)(&args[idxArgs]), workspaceSize, MKIRT_MEM_HBM);
            MKI_CHECK(ret == MKIRT_SUCCESS,
                "Mki RtFunction MallocDevice workspace fail",
                return Status::FailStatus(ERROR_LAUNCH_KERNEL_ERROR, "Mki RtFunction Launch fail"));

            kernelInfo.setDeviceAddr(args[idxArgs]);
    ...
    Status UpdateTilingArgs(void **args, const LaunchParam &launchParam, KernelInfo &kernelInfo,
        const MiniVector<uint64_t> &kernelArgsIndex) const
    {
        ...
        auto ret = Mki::MkiRtMemMallocDevice((void **)(&args[idxArgs]), tilingByteSize, MKIRT_MEM_HBM);
        kernelInfo.setDeviceAddr(args[idxArgs]);
        MKI_CHECK(ret == MKIRT_SUCCESS,
            "Mki RtFunction MallocDevice tiling fail",
            return Status::FailStatus(ERROR_LAUNCH_KERNEL_ERROR, "Mki RtFunction Launch fail"));
        ret = Mki::MkiRtMemCopy((void *)(args[idxArgs]),
            tilingByteSize,
            (void *)(kernelInfo.GetTilingHostAddr()),
            tilingByteSize,
        ...
        current[SPACE_IDX_TWO] = 1;
        MKI_CHECK(ret == MKIRT_SUCCESS,
            "Mki RtFunction GetC2cCtrlAddr fail",
            return Status::FailStatus(ERROR_LAUNCH_KERNEL_ERROR, "Mki RtFunction Launch fail"));
        return Status::OkStatus();
    }
